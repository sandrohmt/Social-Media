...
export const CreateForm = () => { 
  ...
  const schema = yup.object().shape({
    ...
    description: yup.string().required("You must add a description").max(144)
  }) // Configurações para validação de formulário
  ...
  const onCreatePost = async (data: CreateFormData) => { // Passar os dados do post (title e description) pro banco de dados do firebase
    await addDoc(postsRef, {
      ...data, // que é igual a title: data.title, description: data.description
      username: user?.displayName,
      userId: user?.uid // uid é o id usado pelo google
    })
    navigate("/")
  }
